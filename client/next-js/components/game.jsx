...
export function Game({models, sounds, textures, matchId, character}) {
    ...
    useLayoutEffect(() => {
       ...
       function getAimDirection() {
        ...
        function applyKnockbackEffect(playerId, sourcePosition, strength = 5) {
            if (playerId === myPlayerId) {
                const player = players.get(myPlayerId)?.model;
                if (!player) return;
                const dir = new THREE.Vector3().subVectors(player.position, sourcePosition);
                dir.y = 0;
                if (dir.lengthSq() === 0) return;
                dir.normalize();
                playerVelocity.add(dir.multiplyScalar(strength));
            }
        }
                            ...
                            if (message.id !== myPlayerId) {
                                ...
                                if (caster) {
                                    ...
                                    if (myPos && casterPos && myPos.distanceTo(casterPos) < FIRE_RING_RANGE) {
                                        applyKnockbackEffect(myPlayerId, casterPos);
