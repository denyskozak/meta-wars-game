...
export function Game({models}) {
    ...
    useLayoutEffect(() => {
        ...
        let model = models['character'], mixer, idleAction, walkAction, runAction, camera;
        let hp = 100, mana = 100
        let actions = [];
        let settings;
        ...
        function updateModel() {
            if (model) {
                ...
                if (modelMurloc) {
                    const murlocToPlayer = new THREE.Vector3();
                    murlocToPlayer.subVectors(playerPosition, modelMurloc.position).normalize();
                    const murlocTargetRotationY = Math.atan2(murlocToPlayer.x, murlocToPlayer.z);
                    modelMurloc.rotation.y = THREE.MathUtils.lerp(modelMurloc.rotation.y, murlocTargetRotationY, 0.1);
                }
            }
        }
